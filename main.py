"""Installable AstrBot Star: Feishu Agent Card, version 0.2.12."""
import asyncio
import json
import os
import time
...
class FeishuAgentCard(Star):
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.logger.info("Feishu Agent Card 0.2.12 ready (AstrBot 4.28.0 / 4.28.1)")
        except Exception as exc:
    ...
    async def request(self, event, req):
        ...
        if session:
            ...
            presentation = ("飞书卡片展示格式：最终回答请以一行简短、准确概括本次回答主题的 Markdown 二级标题"
                            ...
                            "当前会话的文字回答由此卡片统一交付，不要再调用 send_message_to_user 发送相同文字；媒体及跨会话发送仍可使用该工具。")
